...
class SinglePulsarRedNoise:
    ...
    * ``orf_params``     are the varied ORF parameters (absent when the ORF is
    ...
    def __init__(
        ...
        # ------------------------------------------------------------------ #
        if has_irn:
            assert int_bins is not None and f_intrin is not None, (
                "int_bins and f_intrin must be supplied when irn_psd_func is given."
            )
            ...
            self.f_intrin = f_intrin if f_intrin.ndim == 2 else jnp.broadcast_to(f_intrin, (self.Npulsars, self.int_bins)) 
        ...
        if has_irn:
            ...
            self.last_irn_bin_index = first_irn_bin_index + int_bins
        ...
        if has_irn:
            self.irn_param_container, self.irn_varied_indxs = _parse_psd_func(
                irn_psd_func, irn_helper_dictionary, int_bins
            )
    ...
    def _eval_irn_psd_all(self, irn_params_flat):
        ...
        Returns
        -------
        jnp.ndarray, shape (int_bins, Npulsars)
        """
        per_psr = irn_params_flat.reshape(self.Npulsars, self.n_irn_varied)

        def _single(params, freqs_):
            filled = self.irn_param_container.at[self.irn_varied_indxs].set(params)
            return self.irn_psd_func(freqs_, self.df, *filled)

        return jax.vmap(_single)(per_psr, self.f_intrin).T  # → (int_bins, Npulsars)
    ...
    def _eval_dm_psd_all(self, dm_params_flat):
        ...
        def _single(params, freqs_):
            ...
            return self.dm_psd_func(freqs_, self.df, *filled)

        return jax.vmap(_single)(per_psr, self.f_dm).T  # → (dm_bins, Npulsars)
    ...
    def get_phi_diag(self, xs):
        ...
        n_total = self.int_bins if self.has_irn else 0
        if self.has_dm:
    ...
    * ``orf_params``     are the varied ORF parameters (absent when the ORF is
    ...
    def __init__(
        ...
        if has_irn:
            ...
            self.int_bins = int_bins if has_irn else crn_bins
            self.f_intrin = f_intrin if f_intrin.ndim == 2 else jnp.broadcast_to(f_intrin, (self.Npulsars, self.int_bins)) 
    ...
    def _eval_gwb_psd(self, gwb_psd_params):
        ...
        return self.gwb_psd_func(self.f_common, self.df, *filled)
    ...
    def get_phi_diag(self, xs):
        ...
        n_total = self.int_bins if self.has_irn else self.crn_bins
        if self.has_dm:
            n_total = n_total + self.dm_bins
...
class PowerLawRedNoise(object):
    """
    A class to take care of prior and the phi-matrix construction
    ...
    def get_phi_mat_full(self, xs):
        ...
        if self.orf_fixed:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
            phi = phi.at[self.KGW, self.I, self.J].set(self.orf_val * psd_common)
            return phi.at[self.KGW, self.J, self.I].set(self.orf_val * psd_common)
        else:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
            phi = phi.at[self.KGW, self.I, self.J].set(
                self.orf_func(self.xi, *xs[self.gwb_psd_params_end_idx :]) * psd_common
            )
            return phi.at[self.KGW, self.J, self.I].set(
                self.orf_func(self.xi, *xs[self.gwb_psd_params_end_idx :]) * psd_common
            )
    ...
    def partial_reparam_helper(self, xs):
        ...
        phi_diag_IR = self.powerlaw(log10amp, gamma)

        phi = jnp.zeros((self.crn_bins, self.Npulsars, self.Npulsars))
        phi = phi.at[:, self.diag_idx, self.diag_idx].set(psd_common)
        if self.orf_fixed:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
            phi = phi.at[self.KGW, self.I, self.J].set(self.orf_val * psd_common)
            return (
                phi.at[self.KGW, self.J, self.I].set(self.orf_val * psd_common),
                psd_common, phi_diag_IR
            )
        else:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
            ...
            return (
                phi.at[self.KGW, self.J, self.I].set(
                    self.orf_func(self.xi, *xs[self.gwb_psd_params_end_idx :])
                    * psd_common
                ),
                psd_common, phi_diag_IR
...
class FreeSpecRedNoise(object):
    ...
    def partial_reparam_helper(self, xs):
        ...
        phi_diag_IR = self.free_spectrum_psd_func(irn_psd_params.reshape(self.Npulsars, self.int_bins).T)
